# DLH-1B-R1 — Kernel Equation and Dependency Map (Actual Implemented Behavior, Terminology Corrected)
...
1. **Frozen calibration in code** — `SteadyStateConfig.validate()`/`TransitionConfig.validate()` reject any deviation (calibration not parameterizable).
...
Actual behavior is read from code; comments/docs/status labels and old Matlab provenance are claims only. The source declares D2 (machine diagnostics, no Results authority) throughout — matching the scaffolding (Results/eligibility/submission flags are `False`). **Numerical convergence and scientific validity remain unverified by this audit** (zero execution performed).

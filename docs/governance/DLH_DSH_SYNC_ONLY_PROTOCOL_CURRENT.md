...
Owner 完成项目源替换后，发送此协议的启动 prompt。若上传是否完成未知，只做 GitHub/local 读取并将项目源确认标为 PENDING，不能宣布三端同步完成。

## 允许操作

1. 在 `D:\deep-learning-hank` 核对 cwd、origin、branch、dirty/staged/untracked/ahead-behind；保留所有工作成果。
2. `git fetch origin`，记录 fresh origin/main。读取其 AGENTS、规则索引/必读规则、Owner decision、锁 manifest、Task Index、Snapshot、Roadmap、两份合同及当前交接。
...
报告 pre/post local SHA、fresh origin/main、工作区/分支状态、读取文件、锁/hash 结果、Owner 项目源确认状态、实际命令和模型/算子/训练/pytest 调用数（必须为 0）。用自己的简短表述确认：只学条件目的地份额；m/ell 不学；方向行归一；HA 默认不改；selected-Q 暂停；KFE/GE 未授权；新模型不得改 Owner 路线。

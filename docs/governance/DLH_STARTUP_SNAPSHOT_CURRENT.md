...
### Issue #73 exact execution scope

This is a **bounded one-step scientific numerical execution** — the first Issue
authorized to take an actual numerical step since Issue #63. Its ceiling is
exactly: ONE deterministic reconstruction of the accepted `V_*`; exactly ONE
baseline Route-A `final=False` selected-`Q` build; ONE baseline residual/Jacobian;
ONE bounded lexicographic candidate search; at most ONE accepted experimental
candidate; immediate STOP at the first acceptable candidate or fail-closed on
authorized-search exhaustion; ONE deterministic repeat.
...
Boundary handling uses accepted full-wall-gradient semantics, stacking
**multiple** active gradients and never averaging them; candidate-state domain
safety is authoritative (`min p_b >= 1e-12`). Every candidate must be evaluated
after a **full reassembly** of the same Route-A selected generator — no raw-drift
alternate `Q`, no partial `Q` update, no dual-`Q` comparison.
...
`k_alpha = 0..20`. No outcome-driven tuning and no extra rung. The actual number of
attempted candidates must be reported.

Acceptance requires ALL of: finite; `min p_b(V_trial) >= 1e-12`;
...
attempted `alpha`. The stronger historical material-reduction ratio `<= 0.5` must
be **reported** but is **not** required for one-step acceptance.
...
- using the residual / Jacobi direction;
...
- any Builder scientific branch beyond the dedicated Issue #73 branch (not yet
  created);
...
ONLY task that synthesized accepted Issues #60–#71 evidence, compared four
candidate numerical families, and froze a complete executable bounded solver
contract — while honestly declining to select a single route, because three
...
- policy reproduction = `20 / 20` with controls / utility / drifts / rates
  differences `0`;
- `DOMINANT_COMPONENT_IDENTIFIED = NO`; `MIXED_OR_UNRESOLVED = YES`;
- mean top-20 cancellation ratio = `0.006665998653866562`;
- full repository suite: `693 passed`, `0 failed`, `0 errors`, `6` pre-existing
  oracle warnings;
- deterministic repeat identical.

Exit condition for this acceptance (binding): **HJB convergence = FALSE**. The
acceptance identifies a **mixed fixed-point imbalance** only and does **not**
select a solver design; no new HJB iterate and no new trajectory is accepted.
Owner Route A remains binding. Stationary KFE remains **NOT AUTHORIZED**.
...
lexicographic candidate search over `k_lambda`, `k_Delta`, `k_alpha` each `0..20`
with the policy-frozen regularized Newton direction
`(J + lambda_k*||diag(J)||inf I) d = -R` and `J = rho I - Q`; at most ONE accepted
...
tests plus the full repository suite — inside the exact four-path allowlist.
...
selection; modifying F1–F11 boundary semantics; economics / prices / grid / domain
/ initialization / calibration / tolerances / `PB_MARGIN` / Bellman-tolerance
change; convergence-criterion change; using the residual / Jacobi direction; adding
or tuning ladder rungs; accepting a second outer step; running a trajectory;
claiming HJB convergence; KFE / stationary KFE / steady state /
`solve_household_steady_state`; SCC/global-Q;
GE/multi-region/neural/nominal/calibration/policy/welfare/Results; successor
Issue activation; any Builder scientific branch beyond the dedicated Issue #73

"""DLH-5I (Issue #35) — coupled liquid-domain frontier diagnostic.

Maps the coupled b-extent x mature-a-resolution frontier while preserving the
accepted household controlled process:
...
- only the two mature illiquid resolutions ``a77`` (``da=(10/19)/4``) and ``a153``
  (``da=(10/19)/8``);
- liquid spacing frozen ``db=7/19``; only the three pre-frozen b extents
  ``b60 [-2,375/19]``, ``b80 [-2,515/19]``, ``b100 [-2,655/19]``;
- exactly six variants I0-I5 (Cartesian ``{a77,a153} x {b60,b80,b100}``), fresh
  initialization each, no warm-start, no adaptive seventh grid, no b-resolution
  change, no a-domain widening, no retuning, no clipping.
...
``NOT_AUTHORIZED__DLH_5I_POLICY_ONLY_COUPLED_FRONTIER_DIAGNOSTIC``.
"""
...
# ---------------------------------------------------------------------------
# Terminal classifications / markers (Issue #35 section 11)
# ---------------------------------------------------------------------------

TERMINAL_OUTCOME_A = "DLH_5I_COUPLED_PREFROZEN_DOMAIN_REACHES_CROSS_A_RESOLUTION_JOINT_BOUNDARY_COMPATIBILITY__GPT_REVIEW_REQUIRED"
TERMINAL_OUTCOME_B = "DLH_5I_JOINT_BOUNDARY_COMPATIBILITY_NOT_ROBUST_ACROSS_A_RESOLUTION__SCIENTIFIC_REVIEW_REQUIRED"
TERMINAL_OUTCOME_C = "DLH_5I_COUPLED_B_EXTENT_ATTENUATION_CONFIRMED__COMMON_THRESHOLD_NOT_REACHED__GPT_REVIEW_REQUIRED"
TERMINAL_OUTCOME_D = "DLH_5I_COUPLED_BOUNDARY_BEHAVIOR_PERSISTENT_OR_NONMONOTONIC__SCIENTIFIC_REVIEW_REQUIRED"
TERMINAL_OUTCOME_E = "BLOCKED_DLH_5I_HJB_NUMERICAL_STABILITY"
TERMINAL_OUTCOME_F = "BLOCKED_DLH_5I_REPRODUCIBILITY"
ANNOTATION_CROSS_A_SENSITIVITY = (
    "DLH_5I_CROSS_A_POLICY_RESOLUTION_SENSITIVITY_REMAINS_MATERIAL__NUMERICAL_REVIEW_REQUIRED"
)
...
NOT_AUTHORIZED_MARKER = "NOT_AUTHORIZED__DLH_5I_POLICY_ONLY_COUPLED_FRONTIER_DIAGNOSTIC"
...
# Accepted DLH-5H (Issue #34) facts that I0/I3 must reproduce (read from the
# accepted evidence files DLH_5H_LIQUID_REGRESSION_DIAGNOSTICS.csv). I0 = H2
# (a77,b60); I3 = H3 (a153,b60).
ACCEPTED_H2_UPPER_B_REQUESTED_MAX = 3.915648627e-01
ACCEPTED_H2_UPPER_B_REQUESTED_COUNT = 7
ACCEPTED_H2_UPPER_B_REQUESTED_ARGM_INDEX = (59, 76, 1)
ACCEPTED_H3_UPPER_B_REQUESTED_MAX = 4.449370735e-01
ACCEPTED_H3_UPPER_B_REQUESTED_COUNT = 13
ACCEPTED_H3_UPPER_B_REQUESTED_ARGM_INDEX = (59, 152, 1)

VARIANT_IDS = [
    "I0_A77_B60", "I1_A77_B80", "I2_A77_B100",
    "I3_A153_B60", "I4_A153_B80", "I5_A153_B100",
]
A_RES_ORDER = ["a77", "a153"]
B_EXT_ORDER = ["b60", "b80", "b100"]
...
class DLH5IConfig:
    ...
    h2_h3_repro_tol: float
    output_root: str


def load_config(path: str | pathlib.Path) -> DLH5IConfig:
    ...
    return DLH5IConfig(
        ...
        h2_h3_repro_tol=float(v["h2_h3_repro_tol"]),
...
def _a_pts(cfg: DLH5IConfig, a_res: str) -> int:
...
def _b_ext(cfg: DLH5IConfig, b_ext: str) -> BExt:
...
def build_variant_grid(spec: GridSpec, cfg: DLH5IConfig, z, switch) -> MatlabFaithfulHJBGrid:
...
def grid_plan_identity(cfg: DLH5IConfig) -> dict:
    ...
    # b extents at frozen db0
    b_plan = {}
    for e in cfg.b_extents:
        db = (e.b_hi - cfg.b_lo) / (e.b_pts - 1)
        assert abs(db - DB0) <= 1e-12
        b_plan[e.id] = {"b_pts": e.b_pts, "b_lo": cfg.b_lo, "b_hi": e.b_hi, "db": float(db)}
    assert abs(b_plan["b60"]["b_hi"] - 375.0 / 19.0) <= 1e-12
    assert abs(b_plan["b80"]["b_hi"] - 515.0 / 19.0) <= 1e-12
    assert abs(b_plan["b100"]["b_hi"] - 655.0 / 19.0) <= 1e-12
    assert b_plan["b60"]["b_pts"] == 60 and b_plan["b80"]["b_pts"] == 80 and b_plan["b100"]["b_pts"] == 100
    # per-variant plan
    variants = {}
...
def build_all_grids(cfg: DLH5IConfig, z, switch) -> tuple[dict, dict]:
    plan = grid_plan_identity(cfg)
    grids = {v.id: build_variant_grid(v, cfg, z, switch) for v in cfg.variants}
    g60, g80, g100 = grids["I0_A77_B60"], grids["I1_A77_B80"], grids["I2_A77_B100"]
    g153 = grids["I3_A153_B60"], grids["I4_A153_B80"], grids["I5_A153_B100"]
    # exact a77/a153 nesting: a77 nodes = every second a153 node at every extent
    for c, f in ((g60, g153[0]), (g80, g153[1]), (g100, g153[2])):
        assert np.allclose(f.a[::2], c.a, atol=1e-12)
    # exact b60/b80/b100 same-spacing nesting on common nodes
    for hi in (g80, g100):
        assert np.allclose(hi.b[:60], g60.b, atol=1e-12)
    assert np.allclose(g100.b[:80], g80.b, atol=1e-12)
    # same-b-extent cross-a grids share identical physical b nodes
    assert np.allclose(g153[0].b, g60.b, atol=1e-12)
    assert np.allclose(g153[1].b, g80.b, atol=1e-12)
    assert np.allclose(g153[2].b, g100.b, atol=1e-12)
...
def run_all_variants(cfg: DLH5IConfig, dlh5b, params, numerics) -> dict:
...
def build_fixture(cfg: DLH5IConfig):
...
# ---------------------------------------------------------------------------
# Phase C — same-a b-extent trends (A77: I0->I1->I2; A153: I3->I4->I5)
# ---------------------------------------------------------------------------
...
def _sequence_analysis(cfg: DLH5IConfig, entries: list[dict]) -> dict:
    ...
    return {
        ...
        "vs_b60_raw": vs_base_raw,
        "vs_b60_requested": vs_base_requested,
...
def extent_trends(cfg: DLH5IConfig, runs: dict) -> dict:
...
# ---------------------------------------------------------------------------
# Phase D — cross-a exact-node policy comparisons (I0/I3, I1/I4, I2/I5)
# ---------------------------------------------------------------------------


def cross_a_policy_comparisons(cfg: DLH5IConfig, runs: dict) -> list:
    hjb_by_id = runs["hjb_results"]
    by_id = {v["variant"]: v for v in runs["variants"]}
    pairs = [
        ("I0_A77_B60", "I3_A153_B60", "b60"),
        ("I1_A77_B80", "I4_A153_B80", "b80"),
        ("I2_A77_B100", "I5_A153_B100", "b100"),
    ]
    ...
    for coarse_id, fine_id, ext in pairs:
        ...
        # shared-interior mask: exclude top two coarse layers in BOTH asset
        # dimensions; include all z. a77 nodes are exactly every second a153 node.
...
# ---------------------------------------------------------------------------
# Phase E — joint boundary compatibility frontier
# ---------------------------------------------------------------------------


def joint_frontier(cfg: DLH5IConfig, runs: dict) -> dict:
    by_id = {v["variant"]: v for v in runs["variants"]}
    per_variant = []
...
# ---------------------------------------------------------------------------
# Phase F — scientific stopping rule + terminal classification
# ---------------------------------------------------------------------------


def policy_sensitivity_annotation(cfg: DLH5IConfig, res: list) -> bool:
...
def overall_terminal(cfg: DLH5IConfig, runs: dict, repro: dict, res: list) -> dict:
...
def compare_variant_records(r1: dict, r2: dict, cfg: DLH5IConfig) -> dict:
...
def reproduce(cfg: DLH5IConfig, dlh5b, params, numerics) -> dict:
...
def write_evidence(root: pathlib.Path, cfg: DLH5IConfig, runs: dict, trends: dict,
                   res: list, frontier: dict, repro: dict, term: dict) -> None:
    ...
    _write_csv(root / "DLH_5I_VARIANT_STATUS.csv",
    ...
    for v in runs["variants"]:
        ...
        for o in _boundary_rows(v, "requested"):
            rows.append([v["variant"], o[1], o[2], "requested_offending", _fmt(o[10]),
                         "", "", "", "", "", "", o[4], o[5], o[6], o[7], o[8], o[9], o[10]])
    _write_csv(root / "DLH_5I_BOUNDARY_DIAGNOSTICS.csv",
    ...
    # 3) EXTENT_TRENDS.csv (A77 and A153 b-extent sequences + ratios + flags)
    rows = []
    for a_res in A_RES_ORDER:
        seq = trends["sequences"][a_res]
        for e in seq["entries"]:
            rows.append([a_res, e["b_extent"], e["variant"], "ENTRY",
        ...
        rows.append([a_res, "RATIO", "vs_b60_raw", "", "", "", "", "", "", "",
                     "", "", "", _fmt(seq["vs_b60_raw"]), ""])
        rows.append([a_res, "RATIO", "vs_b60_requested", "", "", "", "", "", "", "",
                     "", "", "", _fmt(seq["vs_b60_requested"]), ""])
        ...
        rows.append([a_res, "FLAG", "upper_a_compatible_on_all_extents", str(seq["upper_a_compatible_on_all_extents"]),
                     "", "", "", "", "", "", "", "", "", "", ""])
    _write_csv(root / "DLH_5I_EXTENT_TRENDS.csv",
    ...
    _write_csv(root / "DLH_5I_CROSS_A_POLICY_STABILITY.csv",
    ...
    _write_csv(root / "DLH_5I_JOINT_COMPATIBILITY_FRONTIER.csv",
               ["item", "b_extent", "a_res", "variant",
                "requested_upper_a", "requested_upper_b", "marker", "compatible"], rows)

    # 6) REPRODUCIBILITY.json
    with open(root / "DLH_5I_REPRODUCIBILITY.json", "w", encoding="utf-8") as fh:
        json.dump(repro, fh, indent=2, default=str, sort_keys=True)

    # 7) EXECUTION_REPORT.md
    with open(root / "DLH_5I_EXECUTION_REPORT.md", "w", encoding="utf-8") as fh:
        fh.write(_render_report(cfg, runs, trends, res, frontier, repro, term))

    # 8) FORBIDDEN_OPERATION_CHECK.md
    with open(root / "DLH_5I_FORBIDDEN_OPERATION_CHECK.md", "w", encoding="utf-8") as fh:
        fh.write(_render_forbidden_check(cfg, runs, repro, term))


def _render_report(cfg: DLH5IConfig, runs: dict, trends: dict, res: list,
                   frontier: dict, repro: dict, term: dict) -> str:
    lines = []
    lines.append("# DLH-5I — Coupled Liquid-Extent Frontier Diagnostic (Issue #35)")
    lines.append("")
    lines.append("Policy-only diagnostic mapping the coupled b-extent x mature-a-resolution "
                 "frontier. Accepted MATLAB-faithful HJB source is immutable and reused "
                 "read-only.")
    lines.append("")
    ...
    lines.append(f"Frozen economics: `wbar={cfg.wbar}`, `r_a={cfg.r_a}`; physical illiquid domain "
                 ...
                 f"liquid spacing `db={cfg.db:.12f}`; only mature a resolutions a77/a153 and b "
                 f"extents b60/b80/b100; all non-grid objects the accepted DLH-5B/DLH-5E fixture "
    ...
    lines.append("## Same-a b-extent trends (Phase C: I0->I1->I2 and I3->I4->I5)")
    lines.append("")
    for a_res in A_RES_ORDER:
        seq = trends["sequences"][a_res]
        lines.append(f"### a{a_res}")
        lines.append("")
        lines.append("| b extent | variant | raw upper-b max | requested upper-b max | raw count | requested count | argmax physical (a,z) | upper-a compatible |")
        lines.append("|---|---|---|---|---|---|---|---|")
        for e in seq["entries"]:
            lines.append(f"| {e['b_extent']} | {e['variant']} | {_sf(e['raw_max'], '.3e')} | "
                         f"{_sf(e['requested_max'], '.3e')} | {e['raw_count']} | {e['requested_count']} | "
                         f"{e['argmax_physical_a_z']} | {e['upper_a_compatible']} |")
        lines.append("")
        lines.append(f"- adjacent raw attenuation ratios (b60/b80, b80/b100): {seq['adjacent_raw']}")
        lines.append(f"- adjacent requested attenuation ratios: {seq['adjacent_requested']}")
        lines.append(f"- raw ratios relative to b60: {seq['vs_b60_raw']}")
        lines.append(f"- requested ratios relative to b60: {seq['vs_b60_requested']}")
        lines.append(f"- strictly decreasing requested upper-b max over extents: {seq['strictly_decreasing_requested']}")
        ...
        lines.append(f"- first b extent with requested upper-b <= 1e-10: {seq['first_requested_below_threshold']}")
        lines.append(f"- upper-a compatible on every b extent: {seq['upper_a_compatible_on_all_extents']}")
        lines.append("")
    lines.append("These are policy-only trends; any upper-a reactivation on an extended b extent "
                 "is preserved as evidence, not clipped.")

    lines.append("")
    lines.append("## Cross-a exact-node policy comparisons (Phase D)")
    lines.append("")
    lines.append("Three required pairs at common b extents (a77 vs every-second a153, all common "
                 "b nodes, all z, no interpolation). Shared-interior mask excludes the top two "
                 "coarse layers in BOTH asset dimensions. `rel_diff = max_abs / max(1, max|coarse|)` "
                 "is scale-aware.")
    lines.append("")
    for c in res:
        lines.append(f"### {c['comparison']} (b{c['b_extent']})")
        lines.append("")
        if not c.get("reached"):
    ...
    lines.append("Per-variant prerequisite marker: `requested_upper_b <= 1e-10 AND "
                 ...
                 "holds only when BOTH mature a resolutions at the same b extent pass both "
    ...
    lines.append("")
    for e in frontier["extents"]:
        lines.append(f"- b{e['b_extent']} cross-a: a77={e['a77_joint_compatible']}, "
    ...
    lines.append("- no existing tracked file modified; dedicated branch "
                 "`dsh/issue-35-dlh-5i-coupled-boundary-frontier-2026-09-01`; allowlist-only "
                 "additions (3 artifacts + 8 evidence files).")
    lines.append("")
    lines.append("DLH-5I implements NO repair and NO stationary acceptance: accepted HJB/KFE/regional "
                 "source immutable; physical a-domain/a_max/taper/economics/tolerances/initialization "
                 "frozen; a77/a153 only; db=7/19 only; no clipping; no D1-D3; no regional or "
                 "multi-province GE; no learned network; no nominal HANK.")
    return "\n".join(lines)


def _render_forbidden_check(cfg: DLH5IConfig, runs: dict, repro: dict, term: dict) -> str:
    lines = [
...
def main(argv: list[str] | None = None) -> int:
    import argparse
    parser = argparse.ArgumentParser(description="DLH-5I coupled liquid-extent frontier diagnostic (Issue #35)")
    parser.add_argument("--config", default="configs/dlh_5i_coupled_boundary_frontier_diagnostic.toml")
    ...
    write_evidence(root, cfg, runs, trends, res, frontier, repro, term)

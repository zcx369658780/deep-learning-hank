"""Canonical one-asset HANK kernel diagnostics layer (DLH-3B-R2).

Evaluates the Issue #15 §7 validation evidence from the kernel output:

- HJB residual (true residual from the solver) + labor KKT + consumption FOC;
- KFE mass error, minimum mass (non-negativity), negative count, NaN count;
- asset clearing residual ``A_hh - B``;
- labor clearing residual ``N_hh - N``;
- accounting residuals: goods, fiscal, profits, wealth flow;
- deterministic reproducibility (two identical solves, max repeat difference).

Also verifies the accepted DLH-3B baseline config identity at load (SHA-256)
and cross-checks the kernel equilibrium against the accepted DLH-3B steady
state (frozen accepted provenance values).
"""
...
class KernelDiagnosticsError(RuntimeError):
    """Raised when the accepted baseline identity does not match the frozen value."""


# Accepted DLH-3B steady state (frozen accepted provenance; Task Index / Startup
# Snapshot / DLH-3B execution report).  The canonical kernel must reproduce this
# equilibrium because it implements the same economic problem (Issue #15 §5:
# "refactor existing HA code if scientific meaning remains unchanged").
...
def run_kernel_diagnostics(config_path: Path) -> KernelDiagnostics:
    """Verify baseline identity, solve the kernel equilibrium, report all gates."""
    ...
    # Gate checks (frozen accepted thresholds from the DLH-3B config).

...
R1 correction: ``generator_min_off_diagonal`` now reports the **literal
minimum over ALL off-diagonal matrix entries**, including implicit sparse
zeros (``min(stored_min, 0.0)``), per Issue #5 semantics.  The minimum
strictly-positive/nonzero stored rate is kept as a separate observation.
"""
...
def solve_household(
    *,
    ...
    # sparse zeros (Issue #5 semantics): if any stored rate is negative, the
    # literal minimum is that negative value; otherwise it is 0.0.
    generator_min_off_diagonal = min(stored_min_off_diagonal, 0.0)
    # Observation only: minimum strictly-positive/nonzero stored rate.

...
Evaluation-count semantics (R1, precise):
  * ``root_trace_evaluations`` = number of full capital evaluations recorded in
    ``root_trace`` (primary endpoint pre-checks + ``brentq`` callback
    evaluations);
  * ``post_root_validation_evaluations`` = the one additional
    ``evaluate_capital(root)`` final verification after ``brentq`` returns;
  * ``total_capital_evaluations`` = the sum of the two.
"""
...
class SteadyStateResult:
    ...
    root_capital: float
    root_converged: bool
    root_trace_evaluations: int
    post_root_validation_evaluations: int
    total_capital_evaluations: int
...
def solve_steady_state(config: SteadyStateConfig) -> SteadyStateResult:
    ...
    # Post-root verification evaluation (not part of the root trace).
    final = evaluate_capital(config, float(root))
    root_converged = bool(root_info.converged)
    root_trace_evaluations = len(trace)
    post_root_validation_evaluations = 1
    total_capital_evaluations = root_trace_evaluations + post_root_validation_evaluations
    return SteadyStateResult(
        config_sha256=config.sha256(),
        root_capital=float(root),
        root_converged=root_converged,
        root_trace_evaluations=root_trace_evaluations,
        post_root_validation_evaluations=post_root_validation_evaluations,
        total_capital_evaluations=total_capital_evaluations,

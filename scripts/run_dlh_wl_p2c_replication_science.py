"""DLH-WL-P2C — durable per-fit-output replication: science runner (Issue #78).

This is the **only** authorized science path for Issue #78. It executes the frozen #75
fit program exactly once and makes every completed fit's *complete scientific output*
durable before the next fit begins. Publication is a separate, strictly zero-fit stage
(``scripts/render_dlh_wl_p2c_replication.py``).

Binding rules implemented here (Issue #78 sections 2, 3, 4, 6):
...
* for every fit: append + flush + fsync ``FIT_ATTEMPT_STARTED``, execute exactly one
  ...
  backend, diagnostics) plus a canonical scientific-payload SHA-256, append + flush +
  fsync ``FIT_COMPLETED``, and only then allow the next fit to begin;
* the runner refuses to start if the P2C ledger already exists, so no cross-invocation
  reset is possible;
...
LEDGER_RELPATH = f"{OUTDIR_RELPATH}/DLH_WL_P2C_ATTEMPT_LEDGER.jsonl"
SEAL_RELPATH = f"{OUTDIR_RELPATH}/DLH_WL_P2C_SCIENCE_SEAL.json"
...
def refuse_if_outputs_exist(ledger_path: Path, seal_path: Path | None = None) -> str | None:
    """Fail closed when a previous P2C ledger (or seal) is present."""
...
def build_completed_record(entry: dict[str, Any], outcome: sci.FitOutcome,
                           ...
                           pre_run_freeze_sha: str) -> dict[str, Any]:
...
def run_one_fit(universe: sci.SyntheticUniverse, entry: dict[str, Any],
    ...
    This is the **only** place in the P2C code base where an optimizer can run. The
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="DLH-WL-P2C science runner (Issue #78)")

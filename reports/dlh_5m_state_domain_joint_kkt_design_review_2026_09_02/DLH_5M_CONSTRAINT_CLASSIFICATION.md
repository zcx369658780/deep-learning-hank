...
| `b <= b_max` (route ceiling; b160 = 1075/19) | **Computational truncation (pure)** | No economic reason bars liquid wealth above `b_max`; the outward liquid drift at this face is a truncation response. This face is the object of the DLH-5M review. | Under R it becomes a constrained face (`mu_b <= 0`); under W it is removed and replaced by the joint-wealth cap. |
| accepted `a_max`-normalized illiquid-return taper | **Numerical stabilization / modeling normalization** | Anchors illiquid-return decay on `a`; not a state constraint. It stabilizes the `a`-face but does not stabilize the `b`-face or a `W`-face. | Unchanged in every candidate; cannot substitute for a boundary law on `b` or `W`. |

---

## Why the classification is decisive

1. **The upper-b face is a truncation, not an economic constraint.** Imposing a
   rectangular tangent-cone law `mu_b <= 0` at `b = b_max` (Design R) converts a
   computational truncation into an economic law. That is only legitimate if the
   Owner accepts the rectangle as the true state domain — which the accepted

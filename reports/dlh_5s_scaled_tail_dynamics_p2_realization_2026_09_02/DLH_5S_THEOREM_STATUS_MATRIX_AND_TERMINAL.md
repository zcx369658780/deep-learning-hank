...
| 6 | Scalar local stability | **EXACT (reduced, homogeneous)** | **unforced** eigenvalue `-7` in s on the lower branch (stable node); NOT a full-HJB rate |
...
| 14 | Coefficient convergence (`Q->K*`, `c/b->0.0175`) | **CONDITIONAL** | no generic full-system rate; homogeneous mean rate `-7` in s only; forcing can dominate (Phase F forcing-rate options) |
...
(vi)  coupled-limit / omega-limit basin condition: the full trajectory enters and
      stays in the basin of the positive p=2 fixed point of the E=0 coupled limit
      system (regular lower sector on the mean mode x damped difference mode)
```

Each is explicit and non-circular (none assumes `Q->K*`, `H->K*`, or
`V_b~K*/b^2`). Under (i)-(vi), the exact scaled machinery gives `E->0`,
asymptotic autonomy, and tracking of the reduced p=2 attractor; the realized
convergence rate is **not** asserted generically (homogeneous `-7` / `-273.67`
only, or explicit forcing-rate option in Phase F). None of (i)-(vi) follows
from S1+S2+S3 alone.
...
1. *The transformed dynamics are useful* — YES: exact scaled HJB, exact vector
   ...
   homogeneous eigenvalue `-7` and reduced basin `(0,1/r_b^2)`, homogeneous
   z-difference damping `-273.67`, and a materially sharper explicit sufficient
   condition set than DLH-5Q's generic gap.
2. *But the controlling unproved object remains the scaled-tail tightness /
   ...
   overclaims), S1+S2+S3 alone do **not** establish (i)-(iii), (v), (vi). In
   particular the trajectory's entry into the reduced attractor's basin is an

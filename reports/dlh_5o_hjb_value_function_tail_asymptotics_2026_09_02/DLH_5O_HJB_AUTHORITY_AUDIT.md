...
- The **post-convergence operator** `post` is assembled from the actual drifts
  `mu_a`, `mu_b` (the `asset_drifts_matlab_faithful` outputs). At convergence both
  operators correspond to the same continuum interior drift `mu_b V_b + mu_a V_a`.
...
converged fixed point and hence the continuum interior HJB identity (A5, conditional
on upwind-semantics/regularity), and the finite-grid closures (A7-A8, numerical
semantics only). It does **not** specify an unbounded-`b` HJB problem: there is **no
asymptotic boundary/transversality condition** and **no tail-scaling statement**
(A9-A11). Therefore any tail asymptotics derived below is a **conditional dominant
balance**, not an unconditional theorem from accepted authority.

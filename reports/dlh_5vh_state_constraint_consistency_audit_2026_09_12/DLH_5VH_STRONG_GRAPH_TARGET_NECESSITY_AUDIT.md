# DLH-5V-H — Strong-Graph-Target Necessity Audit (Micro-Rev Rev 4)

**Sequence step C of Issue #56; Micro-Rev tasks C and F.** Question: is the Issue-#55 raw Kuratowski admissible-drift-set graph target *genuinely necessary* for state-constraint HJB scheme convergence, *merely sufficient / over-strong*, or *unanswered and needing a project-specific lemma*? Issue #55's Outcome C remains accepted under the target it was assigned; this audit answers the distinct necessity question. Rev 4 carries forward the quantifier-corrected half-relaxed-limit algebra, the local-state same-candidate recovery with the true χ algebra, and re-states the transfer chain on the TRUE full control domain (c > 0, l ≥ 0, d ∈ R) with the Hamiltonian regularity on the effective gradient region {p_b ≥ η > 0}; the raw-graph claim stays scoped to exactly the level the operator structure, the toy, and the corrected algebra support.
...
- **Interior-state branch:** H_lim = H_proj (interior recovery + restriction), so ρφ(x̂) ≤ H_proj(x̂, Dφ) — the Soner-form subsolution on the closure (on the effective gradient region).
- **W-contact-state branch:** H_m(s_m, p) ≤ H_proj(s_m, p) — the buffered discrete candidate set is a subset of the TRUE full household control set (c > 0, l ≥ 0, d ∈ R; NO static budget, NO compactness used for this inclusion — it is now trivial); the limiting step limsup_m H_proj(s_m, p_m) ≤ H_proj(x̂, p) uses the USC/continuity of the TRUE Hamiltonian on the effective region {p_b ≥ η > 0} (monotone-scheme report §D.4, established by the local effective-compactness/coercivity lemma from the frozen objective). Hence ρφ(x̂) ≤ H_lim ≤ H_proj(x̂, Dφ) **on the effective gradient region**; the overall transfer is **CONDITIONAL_ON_EFFECTIVE_DOMAIN_REGULARITY** — the condition (relevant viscosity test gradients with p_b ≥ η > 0) belongs to the Outcome-B block. The Rev-3 static-budget Γ is NOT used anywhere.
...
The raw graph target is sufficient but over-strong (trichotomy option 2). The legitimate target is the operator/test-function consistency target of step D; the frozen process passes its consistency components (including the local-state same-candidate recovery bridge with the true χ algebra on the TRUE control domain and the Hamiltonian regularity on the effective gradient region); the boundary subsolution transfer on the closure is closed on the effective region via the trivial restriction + USC argument, with the overall status CONDITIONAL_ON_EFFECTIVE_DOMAIN_REGULARITY; the single bounded Outcome-B gap is the Hamiltonian-effective-domain / state-constraint-comparison application block (relevant-viscosity-gradient restriction + Soner-assumption mapping + theorem application, with the constrained characterization as a coupled sublemma), declared explicitly rather than carried forward.

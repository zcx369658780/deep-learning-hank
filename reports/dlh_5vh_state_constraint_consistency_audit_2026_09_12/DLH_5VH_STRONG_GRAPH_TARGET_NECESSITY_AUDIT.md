# DLH-5V-H — Strong-Graph-Target Necessity Audit (Micro-Rev Rev 5)

**Sequence step C of Issue #56; Micro-Rev tasks C and F.** Question: is the Issue-#55 raw Kuratowski admissible-drift-set graph target *genuinely necessary* for state-constraint HJB scheme convergence, *merely sufficient / over-strong*, or *unanswered and needing a project-specific lemma*? Issue #55's Outcome C remains accepted under the target it was assigned; this audit answers the distinct necessity question. Rev 5 carries forward the quantifier-corrected half-relaxed-limit algebra, the local-state same-candidate recovery with the true χ algebra, and re-states the transfer chain on the TRUE full control domain (c > 0, l ≥ 0, d ∈ R) with the Hamiltonian regularity on the effective gradient region {p_b > 0} — **parameter-generic in γ_c > 0, φ > 0** (no numeric γ_c / φ authority; the fixture instances are VALIDATION_FIXTURE_NOT_CALIBRATION) — and with the pointwise/local-vs-global η distinction; the raw-graph claim stays scoped to exactly the level the operator structure, the toy, and the corrected algebra support.
...
- **W-contact-state branch:** H_m(s_m, p) ≤ H_proj(s_m, p) — the buffered discrete candidate set is a subset of the TRUE full household control set (c > 0, l ≥ 0, d ∈ R; NO static budget, NO compactness used for this inclusion — it is now trivial); the limiting step limsup_m H_proj(s_m, p_m) ≤ H_proj(x̂, p) uses the USC/continuity of the TRUE Hamiltonian on the effective region {p_b > 0} (monotone-scheme report §D.4, established by the local effective-compactness/coercivity lemma — parameter-generic in γ_c > 0, φ > 0; at any fixed test gradient with p_b > 0 a local η < p_b exists). Hence ρφ(x̂) ≤ H_lim ≤ H_proj(x̂, Dφ) **for every effective-domain test gradient (p_b > 0)**; the overall transfer is **CONDITIONAL_ON_EFFECTIVE_DOMAIN_REGULARITY** — the condition being whether all relevant viscosity gradients stay in the finite effective domain (and away from p_b = 0 as the selected comparison theorem requires), which belongs to the Outcome-B block. The Rev-3 static-budget Γ is NOT used anywhere; no global uniform η is demanded a priori.
...
**Remaining for convergence:** passing from the halves ū (subsolution on the closure w.r.t. H_proj) and u̲ (supersolution in the interior) to u_m → V requires the **relevant-viscosity-gradient restriction** and the **comparison/unique-continuation theorem for the project's continuous state-constrained problem** — the single bounded Outcome-B block (§E.6 of the monotone-scheme report; the H_proj regularity on the effective region is established parameter-generically, the exact theorem-application including the Soner compact-control assumption mapping and the gradient restriction remain). The constrained characterization ρV ≤ H_T at the boundary is a coupled sublemma of the same block (not established; not needed for the transfer).
...
The raw graph target is sufficient but over-strong (trichotomy option 2). The legitimate target is the operator/test-function consistency target of step D; the frozen process passes its consistency components (including the local-state same-candidate recovery bridge with the true χ algebra on the TRUE control domain and the Hamiltonian regularity on the effective gradient region, parameter-generic in γ_c > 0, φ > 0); the boundary subsolution transfer on the closure is closed for every effective-domain test gradient (p_b > 0, local η) via the trivial restriction + USC argument, with the overall status CONDITIONAL_ON_EFFECTIVE_DOMAIN_REGULARITY; the single bounded Outcome-B gap is the Hamiltonian-effective-domain / state-constraint-comparison application block (whether all relevant viscosity gradients stay in the finite effective domain and away from p_b = 0 as needed + Soner-assumption mapping + exact theorem application, with the constrained characterization as a coupled sublemma), declared explicitly rather than carried forward.

# DLH-5V-H — Continuous State-Constraint HJB and Viscosity Target (Micro-Rev Rev 5)

**Sequence step A of Issue #56; Micro-Rev tasks A and B.** This report fixes the continuous state-constraint HJB object and its viscosity semantics BEFORE any numerical target is defined, records Soner's actual sign convention and derives the project transform explicitly (Task A), and demotes the tangent-cone-restricted Hamiltonian to an auxiliary object (Task B). Rev 5 (i) defines the TRUE full interior control domain (c > 0, l ≥ 0, d ∈ R — no static budget, no installed hard bounds, τ corrected to its source role as a wage wedge), (ii) audits the TRUE Hamiltonian effective domain **parameter-generically** for every frozen γ_c > 0, φ > 0 (p_b > 0 / p_b = 0 / p_b < 0, with the γ_c-dependence at p_b = p_a = 0), (iii) proves the LOCAL effective-compactness/coercivity lemma on compact gradient sets with p_b ≥ η > 0 **without any numeric γ_c / φ value** (γ_c = 2 / φ = 5 are the canonical `VALIDATION_FIXTURE_NOT_CALIBRATION`, not theory authority), and (iv) distinguishes pointwise/local p_b > 0 from a global uniform η. No grid, aspect or stencil redesign is performed; the frozen geometry is consumed as given.
...
## 4. What the continuous target requires of a scheme (Rev 5)
...
4. A comparison hypothesis for the limit problem (the Outcome-B block: relevant-gradient restriction + Soner-assumption mapping + theorem application; the H_proj regularity on the effective region is established in §3.5 parameter-generically).
